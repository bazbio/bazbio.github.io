...
export function buildFlowMap(data,{expanded=false}={}){
  ...
  for(const b of departments){
    ...
    const subtitle=excluded?'대표님 승인 범위에서 제외':own.length?own.map(a=>displayLabel(a.종류)).filter((v,i,a)=>a.indexOf(v)===i).join(' · '):p?`부서 계획 v${p.버전} · ${p.상태}`:b.상태;
  ...
  add('approval','통합 계획 · 대표님 승인',approvals.length?approvals.map(a=>displayLabel(a.종류)).join(' · '):effective?`유효 계획 v${effective.버전} · 승인`:'부서 계획 취합 후 승인',approvals,effective?'done':'waiting',{detail:data.변경상태?`계획 변경: ${data.변경상태}. 현재 실행 기준은 승인된 유효 계획입니다.`:effective?.요약});
  ...
  if(expanded&&runs.length){
    ...
    const n=add('execution','업무 실행 · 검증',effective?`필수 완료 ${requiredDone}/${required.length} · 전체 ${done}/${runs.length}`:'대표님 승인 후 실행',runActions,fallback,{runs});
  ...
  add('close','대표님 종결 승인',closed?'업무 종결 완료':'최종 결과 승인 후 종결',closeActions,closed?'done':'waiting');
  link('result','close');
...
export function createFlowMap({el,button}){
  function overview(data){
    ...
    const heading=el('div',null,'flow-heading');heading.append(el('h3','전체 업무 흐름 맵'),el('span',displayLabel(data.단계),'badge'));
    ...
    function select(id){
      ...
      if(planned?.length){const list=el('ul');for(const m of planned)list.append(el('li',`${m.제목} · ${m.담당자명} · ${m.시작일} → ${m.완료일}`));inspector.append(el('p','최근 부서 계획 · 실행 기준은 대표님이 승인한 유효 계획입니다.','hint'),list);}
      if(n.runs?.length)inspector.append(el('p','마일스톤 펼치기에서 개별 작업과 선후 관계를 확인하세요.','hint'));
      for(const a of n.actions){const row=el('div',null,'flow-action');row.append(el('strong',`${a.담당부서.이름} · ${a.담당자.표시명} · ${displayLabel(a.종류)}`),el('p',`${new Date(a.처리기한).toLocaleString('ko-KR')}까지${a.기한초과?' · 기한 초과':''}`));if(a.미팅대기)row.append(el('p','미팅 결론·후속 업무 대기','notice'));if(a.마일스톤제목)row.append(el('p',a.마일스톤제목));row.append(button('해당 차례 보기','text-button',()=>jump(a)));inspector.append(row);}
    }
    function render(){
      ...
      summary.replaceChildren(el('strong',model.closed?'종결 완료':model.rejected?'비승인으로 종료':`현재 ${model.actions.length}건의 처리 차례`),el('span',`행동 기한 초과 ${model.actions.filter(a=>a.기한초과).length}건`),el('span',model.effective?`필수 마일스톤 ${model.requiredDone}/${model.required} 완료`:'대표님 승인 전 · 실행 시작 전'));
