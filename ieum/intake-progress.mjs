const states={접수검토:['수락 대기','waiting'],수락:['수락 완료','accepted'],배정완료:['수락 완료','accepted'],보완:['보완 요청','supplement'],비승인:['반려 · 조정 필요','rejected'],제외제안:['제외 제안','excluded']};
export function summarizeIntake(rows){
 const counts={accepted:0,waiting:0,supplement:0,rejected:0,excluded:0,unknown:0};
 for(const row of rows)counts[states[row.상태]?.[1]||'unknown']++;
 return {counts,text:[`수락 ${counts.accepted}/${rows.length}개 부서`,counts.waiting&&`응답 대기 ${counts.waiting}`,counts.supplement&&`보완 ${counts.supplement}`,counts.rejected&&`반려 ${counts.rejected}`,counts.excluded&&`제외 제안 ${counts.excluded}`,counts.unknown&&`상태 확인 ${counts.unknown}`].filter(Boolean).join(' · ')};
}
...
export function createIntakeProgress({el,button,api}){
 function compact(rows){
  ...
  const tags=el('div',null,'intake-tags');for(const row of rows){const [label,tone]=states[row.상태]||[row.상태||'상태 확인','unknown'];tags.append(el('span',`${row.부서명} · ${label}`,`intake-tag ${tone}`));}box.append(tags);return box;
 }
 function overview(data){
  ...
  function render(current){
   ...
   for(const row of rows){
    const [label,tone]=states[row.상태]||[row.상태||'상태 확인','unknown'],card=el('article',null,`intake-row ${tone}`);card.dataset.departmentId=row.부서ID;
    ...
    if(tone==='accepted')card.append(el('p',row.응답?.결과==='수락'?`수락자 ${row.응답.처리자} · ${time(row.응답.시각)}`:'수락 완료 · 수락자 기록 확인 필요','intake-person'));
