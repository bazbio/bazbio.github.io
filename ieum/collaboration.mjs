...
export function createCollaboration({el,button,send,refresh,getInfo,isBusy,message,handleError}){
 ...
 function form(data,sub,kind,args,action){const f=el('form'),err=el('p','','error');err.setAttribute('role','alert');f.append(err);f.addEventListener('submit',async e=>{e.preventDefault();if(isBusy())return;try{err.textContent='';const result=await send({종류:kind,업무ID:data.업무ID,부서업무ID:sub,기대업무개정:data.업무개정,...(action?{행동ID:action.행동ID}:{}),인자:args()});if(result){await refresh(data.업무ID);message('처리가 반영되었습니다. 다음 차례를 확인하세요.');}}catch(error){err.textContent=errorMessage(error);if(error.status===401)handleError(error);if(error.status===409)message('최신 내용을 새로고침한 뒤 다시 확인해 주세요. 입력 내용은 유지됩니다.',true);}});return f;}
